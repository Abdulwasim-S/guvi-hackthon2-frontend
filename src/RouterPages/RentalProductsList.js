...
const RentalProductsList = () => {
  const { rentalProducts } = useContext(AppContext);
  ...
  return (
    <>
      <div className="row w-100 bg-success justify-content-around">
        ...
        <div className="col-md-3"></div>
        {state &&
          rentalProducts.map((data, idx) => (
            <Card
              className="product-card col-md-3 mx-3 my-3 pb-5 justify-content-between"
              id={data.product.toLowerCase()}
              key={idx}
              style={{ width: "18rem" }}
            >
