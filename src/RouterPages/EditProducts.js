...
const EditProducts = () => {
  ...
  async function deleting(id){
    try{const response = await fetch(
      "https://guvi-hackthon2-abdulwasims-backend.vercel.app/rental/products",
      {
        method: "DELETE",
        headers: {
  ...
  return (
    <div className='row bg-success justify-content-around'>
      ...
      {products.map((data,idx)=>(
        <Card className='product-card col-md-3 mx-3 my-3 justify-content-between ' key={idx} style={{ width: '18rem' }} id={data.product.toLowerCase()}>
